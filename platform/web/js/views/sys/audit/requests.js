...
export function requestCard(r, { highlight = false, showEngagement = true } = {}) {
  ...
  return h(`article.card.aud-req${r.overdue ? '.late' : ''}${highlight ? '.flash.selected' : ''}`, { id: `req-${r.id}` },
    ...
    h('h3.aud-req-title', r.title),
    r.details ? h('p.aud-para.muted', r.details) : null,
    ...
    r.response_text ? h('div.aud-reply', h('div.aud-reply-head', icon('messageSquare'), h('strong', L('الرد', 'Reply')), h('span.tiny.faint', `${r.responded_by ? r.responded_by.name_ar : ''} · ${fmtDate(r.responded_at)}`)), h('p.aud-para', r.response_text), docList(r.docs)) : null,
