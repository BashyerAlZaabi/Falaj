// Tasks (#/tasks[/mine|all|overdue|done]): filter tabs with counts, search,
// grouping (due date · project · priority), quick add, collapsible "Done".
import { api } from '../api.js';
import { h, icon, menu, segmented, skeleton, emptyState } from '../ui.js';
import { L, fmtNum } from '../i18n.js';
import { state } from '../state.js';
import { taskRow, dueInfo, countText, PRIORITY, captureFocus, restoreFocus, quickAdd, searchBox, loadError, normalize } from '../work-items.js';
import { newTask } from './projects.js';

const KEYS = ['mine', 'all', 'overdue', 'done'];
...
const GROUPS = () => [['due', L('موعد الاستحقاق', 'Due date')], ['project', L('المشروع', 'Project')], ['priority', L('الأولوية', 'Priority')]];
...
export async function renderTasks(root, params = [], { soft } = {}) {
  ...
  const load = Promise.all(KEYS.map((k) => api(`/api/tasks${QUERY[k]}`)));
...
function build(page, [mine, all, overdue, done]) {
  const lists = { mine: mine.filter((x) => x.status !== 'done'), all, overdue, done };
  ...
  function draw() {
    ...
    const add = ui.filter !== 'done' ? quickAdd({ key: 'quick-tasks', placeholder: L('أضف مهمة لك…', 'Add a task for yourself…'), label: L('أضف مهمة سريعة لك: اكتب العنوان واضغط Enter', 'Add a quick task for yourself: type a title and press Enter'), input: {} }) : null;
    ...
    if (!rows.length) { results.replaceChildren(...[add, empty(found, needle, search)].filter(Boolean)); return; }
    results.replaceChildren(...[add, ...grouped(rows).map(section), foot].filter(Boolean));
  }
  draw();
}
